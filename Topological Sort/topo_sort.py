...
Topological Sort Implementation
  ...
  File: topo_sort.py
  Description: This code preforms various functions on a 
  graph such as checking to see if it has a cycle and 
  sorting the graph using a topological sort.
...
class Graph(object):
    ...
    def get_adj_back_vertex(self, v):
        """get back adj vertex."""
        verts = []
        n_vert = len(self.vertices)
        for i in range(n_vert):
    ...
    def has_cycle(self):
        ...
        for thing in self.vertices:
            if self.dfs(self.get_index(thing.get_label())):
                return True
    ...
    def delete(self, vertex_label):
        """
        Delete all edges from the adjacencey matrix
        """
        idx = self.get_index(vertex_label)
        #for col in self.adj_mat[idx]:
        for x in range(len(self.adj_mat[idx])):
            self.adj_mat[idx][x] = 0
    ...
    def toposort(self):
        ...
        topo_list = []
        level = Queue()
        deleted_vert_list = set()
        for thing in self.vertices:
            # print(self.get_adj_back_vertex(self.get_index(thing.get_label())))
            if not self.get_adj_back_vertex(self.get_index(thing.get_label())):
                level.enqueue(thing)
                topo_list.append(thing.get_label())
                deleted_vert_list.add(self.get_index(thing.get_label()))
        while not level.is_empty():
            current = level.dequeue()
            curr_edges  = self.get_adj_vertexes(self.get_index(current.get_label()))
            deleted_vert_list.add(self.get_index(current.get_label()))
            self.delete(current.get_label())
            for i in curr_edges:
                edges_point_to_i = self.get_adj_back_vertex(self.get_index\
                                                            (self.vertices[i].get_label()))
                if len(edges_point_to_i) == 0:
                    level.enqueue(self.vertices[i])
                    topo_list.append(self.vertices[i].get_label())
            curr_edges = []
        return topo_list
